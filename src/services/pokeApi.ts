...
import type { PokemonListResponse, PokemonDetails, PokemonListItem, PokemonTypeSlot } from '../types/pokemon.types';
...
export const getAllTypes = async (): Promise<ApiResource[]> => {
    try {
        const response = await axios.get<ApiResourceListResponse>(`${API_BASE_URL}/type`, { params: { limit: 50 } });
...
export const getAllGenerations = async (): Promise<ApiResource[]> => {
    try {
        const response = await axios.get<ApiResourceListResponse>(`${API_BASE_URL}/generation`, { params: { limit: 20 } });
        return response.data.results;
    } catch (error) {
...
export const getPokemonsByType = async (typeName: string): Promise<PokemonListItem[]> => {
    try {
        const response = await axios.get<{ pokemon: PokemonTypeSlot[] }>(`${API_BASE_URL}/type/${typeName}`);
...
export const getPokemonsByGeneration = async (generationName: string): Promise<PokemonListItem[]> => {
    try {
        const response = await axios.get<{ pokemon_species: PokemonListItem[] }>(`${API_BASE_URL}/generation/${generationName}`);
