        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
            <Col xs={24} sm={12}>
                <Select
                    ...
                    options={types.map(type => ({ value: type.name, label: t(`pokemonTypes.${type.name}`) }))}
                />
            </Col>
            <Col xs={24} sm={12}>
                <Select
                    ...
                    options={generations.map(gen => ({ value: gen.name, label: gen.name.replace('generation-', '').toUpperCase() }))}
                />
            </Col>
        </Row>
    );
