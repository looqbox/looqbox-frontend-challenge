...
const HomePage: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (!selectedType && !selectedGeneration) {
                ...
                const genPromise = selectedGeneration ? getPokemonsByGeneration(selectedGeneration) : Promise.resolve(null);
    ...
    const renderContent = () => {
        ...
        if (isLoadingData) {
            return <Row justify="center" align="middle" style={{ minHeight: '300px' }}><Spin size="large" /></Row>;
        }
        ...
        if (displayedPokemons.length === 0 && (selectedType || selectedGeneration)) {
            return <Result status="info" title={t('home.noResults')} subTitle={t('home.noResultsSubtitle')} />
        }
    ...
    return (
        <>
            ...
            {error && <Alert message={t('home.error.title')} description={error} type="error" showIcon closable onClose={() => setError(null)} style={{ marginBottom: 24 }} />}

            {renderContent()}

            {!selectedType && !selectedGeneration && status !== 'failed' && displayedPokemons.length > 0 && (
